...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define STACK_PATTERN2 0xDEADDEAD

// Declare the stack as an array, initializing the first few bytes with a known pattern
uint32_t stack2[256] __attribute__((aligned(8))) = {STACK_PATTERN2};
typedef struct
{
    uint32_t r0;
    uint32_t r1;
